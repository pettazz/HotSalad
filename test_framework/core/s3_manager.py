...
class S3LoggingBucket(object):
    ...
    def __init__(self, 
        ...
        if not log_bucket:
            log_bucket = constants.s3ManagerAccount.LOG_BUCKET
        if not bucket_url:
            bucket_url = constants.s3ManagerAccount.BUCKET_URL
        if not selenium_access_key:
            selenium_access_key = constants.s3ManagerAccount.SELENIUM_ACCESS_KEY
        if not selenium_secret_key:
            selenium_secret_key = constants.s3ManagerAccount.SELENIUM_SECRET_KEY
