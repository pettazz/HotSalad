...
Currently Selenium will open up a new Firefox
window to handle Gmail.

Note: Make sure that whatever Email address you use has HTML-view enabled
as its default view (the other view uses too many IFrames for Selenium's 
taste)."""
...
class EmailManager:
    ...
    em = EmailManager()
    result = em.check_for_recipient("some_dude_bro_67@gmail.com")
    """
    ...
    def __init__(self, uname=None, pwd=None,
        ...
        if not uname:
            uname = constants.EmailManagerAccount.USERNAME
        if not pwd:
            pwd = constants.EmailManagerAccount.PASSWORD
        self.uname = uname
        self.pwd = pwd
